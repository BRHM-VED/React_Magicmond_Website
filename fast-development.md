...
### ❌ BEFORE (Raw Figma-to-Code Output)
```tsx
// A massive, single-file page with hardcoded classes, text, and styling
export default function HomePage() {
  return (
    <div className="w-full bg-[#0c4a6e] py-20 px-8 flex flex-col items-center">
      <h1 className="text-[56px] text-white font-extrabold tracking-tight text-center leading-[1.1]">
        Build The Infrastructure of Tomorrow
      </h1>
      <p className="text-[18px] text-[#f0f9ff]/80 mt-4 max-w-xl text-center">
        Yamuna Infra delivers premium real estate development solutions.
      </p>
      <button className="mt-8 bg-[#0284c7] hover:bg-[#0284c7]/90 text-white font-bold py-4 px-8 rounded-lg shadow-lg flex items-center gap-2">
        Explore Projects
...
###  AFTER (Refactored Clean-Code MVC Style)

**1. Create Component (`src/components/home/HeroSection.tsx`)**
```tsx
...
export const HeroSection: React.FC = () => {
  return (
    <section className={`w-full py-20 px-8 flex flex-col items-center`} style={{ backgroundColor: COLORS.primary[900] }}>
      <h1 className={`${FONTS.heading} ${FONTS.sizes.h1} text-white text-center leading-[1.1]`}>
        {STRINGS.home.heroTitle}
      </h1>
      <p className={`${FONTS.body} ${FONTS.sizes.body} mt-4 max-w-xl text-center`} style={{ color: `${COLORS.primary[50]}cc` }}>
...
**2. Assemble Page (`src/pages/Home.tsx`)**
```tsx
import React from 'react';
...
const Home: React.FC = () => {
  // Model state fetch if applicable
  const { data: homeData, error, isLoading } = useSWR('/api/home-content', fetcher);

  return (
    <main className="w-full min-h-screen">
      <HeroSection />
      <FeaturesSection data={homeData?.features} isLoading={isLoading} error={error} />
    </main>
  );
};
